...
function GitHubStarsButton({
  ...
  useEffect(() => {
    ...
    // Check for both common token names in Vite environment
    const token = import.meta.env.VITE_GITHUB_TOKEN || import.meta.env.VITE_GH_TOKEN;
    if (token) {
      headers['Authorization'] = `Bearer ${token}`;
    }

    // Add a timestamp query parameter to bypass cache without needing the Cache-Control header
    const timestamp = Date.now();
    fetch(`https://api.github.com/users/${username}/repos?per_page=100&t=${timestamp}`, { headers })
      .then((response) => response.json())
      .then((data) => {
        if (Array.isArray(data)) {
          const totalStars = data.reduce((acc: number, currRepo: { stargazers_count?: number }) => acc + (currRepo.stargazers_count || 0), 0);
          setStars(totalStars);
        }
      })
      .catch(console.error)
  ...
  return (
    <motion.a
      ...
      {...props}
    >
      <div className='flex items-center gap-2 px-4 py-2 h-full has-[>svg]:px-3'>
